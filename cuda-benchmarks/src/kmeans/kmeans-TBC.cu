...
__device__ void load_k_smem(int* smem, const int smem_size, int* k_centers, const int k, const int j)
{
    ...
    {
        int index = btr + l * block.num_threads() + j * smem_size * tbc_size + smem_size * cbr;
...
/**
 * Calculates kmeans in shared_memory.
 * @param smem_size Size of shared memory.
 * @param input_iterations The number of partitions the algorithm has to break the input, because there are not enough threads in the grid.
 * @param input The input values / data points.
 * @param input_size The size of the input array.
 * @param k The number of clusters in the kmeans.
 * @param k_centers Holds the current cluster centers for each cluster.
 * @param k_means Stores the mean for the k clusters at the end of each iteration of kmeans.
 * @param k_counter Counts the number of data points that belong to a cluster.
 * @param k_smem_iterations The number of partitions the algorithm has to break k, because it doesn't fit into shared_memory.
 * @param k_iterations The number of partitions the algorithm has to break k, because there are not enough threads in the grid.
 */
...
{   
    ...
    {   
        int value = 0;
        if (grid.thread_rank() + i * grid.num_threads() < input_size)
        ...
        for (int j = 0; j < k_smem_iterations; j++)
        {
            ...
            if (grid.thread_rank() + i * grid.num_threads() < input_size)
            {
                ...
                for (int l = 0; l < tbc_size && k_left > 0; l++)
                {
                    const int* dst_smem = cluster.map_shared_rank(smem, l);

                    int current_min_dist = (value - dst_smem[0])
                                         * (value - dst_smem[0]);
        ...
        if (grid.thread_rank() + i * grid.num_threads() < input_size)
        {
            atomicAdd(&k_means[min_index], value);
            atomicAdd(&k_counter[min_index], 1);
        }
    }
    ...
    for (int i = 0; i < k_iterations; i++)
    {
        ull index = grid.thread_rank() + i 
                  * grid.num_threads();
        if (index < k) 
            k_means[index] = k_means[index] / k_counter[index];
    }
}
...
{   
    ...
    num_blocks = min(num_blocks, 104); // Kernel doesn't launch for some reason with higher number of blocks.
    ...
    if (checkCmdLineFlag(argc, (const char**) argv, "CUDA_Cluster_Size"))
        tbc_size = getCmdLineArgumentInt(argc, (const char**) argv, "CUDA_Cluster_Size");

    if (num_blocks == 0 || num_blocks > 104)
    {
    ...
    if (num_blocks % tbc_size)
    {
        printf("Number of Blocks %d is not divisible by CUDA cluster size %d\n", num_blocks, tbc_size);
        exit(EXIT_FAILURE);
    }
    /*
    if (k * 10 > input_size)
    {
        std::cout << "Too many kmeans-clusters in order to initialize properly." << std::endl;
        exit(EXIT_FAILURE);
    } */
    ...
    */
    ...
    CUDA_CHECK(cudaMalloc((void**) &d_k_means, k * sizeof(int)));
    ...
    int k_smem_iterations =  k / (smem_size_int * tbc_size);
    k_smem_iterations = k % (smem_size_int * tbc_size) ? k_smem_iterations + 1 : k_smem_iterations;
    ...
    while (true)
    {
        ...
        CUDA_CHECK(cudaMemcpy(cuda_result, d_k_means, k * sizeof(int), cudaMemcpyDeviceToHost));

        if (cmp_array(cuda_result, k_centers, k))
            break;
        ...
        copy_array(cuda_result, k_centers, k);
    }
