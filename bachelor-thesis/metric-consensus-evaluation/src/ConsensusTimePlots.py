...
def calculateMean(n : int, rounds : list[tuple]) -> float:
    ...
    filteredList = list(map(lambda x: float(x[1]), filteredList))
...
def convertToDiagram(group : list, data : list[tuple], ax):
    for val in group:
        ...
        filteredData = list(filter(lambda x: float(x[3]) == val, data))
        #Calculate Values
        ...
        print(roundsMean)
        #Create graph
        ax.plot(participants, roundsMean, label= r'$\epsilon =$ ' + str(val))
...
def normalize(n : int, consensusTime : int, synchronous : bool) -> float:
    if(synchronous):
        ...
        return consensusTime / n #/ (n * log(n))
...
def plotSettings(ax : Axes, dirName : str):
    ...
    ax.set_xscale('log')
    ax.grid(True)
    ax.set_title(r'Mean-Value - Asynchronous')
...
def createDiagrammFromDirectory(path : str, shouldSave : bool = False):
    ...
    if(shouldSave):
        ...
        fig.savefig('plots/svg/' + dirName + '.svg')
    else:
        plt.show()
...
def createDiagrammFromFile(path : str):
    ...
    if(len(data[0])):
        ...
        group = list(map(lambda x: float(x[3]), data))
        ...
        fig.set_size_inches(5.1, 3.7)
        plt.show()
        fig.savefig('plots/svg/' + 'MeanValue - Asynchronous' + '.svg')
...
def main():
    ...
    file : str = 'Mean-Value_R-100_SYNC-false.csv'
